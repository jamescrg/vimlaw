...
def cycle_user_filter(request, session_key, direction):
    ...
    Drives the tasks/time/expenses/flat-fees "[ / ]" keyboard shortcut:
    the active users in username order, wrapping at the ends, with NO
    "All users" stop — from All Users the cycle is entered at the
    first/last user. All Users stays reachable from the dropdown.
