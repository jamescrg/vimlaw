...
def documents_edit(request, document_id):
    ...
    if request.method == "POST":
        ...
        if uploaded_file and document.drive_file_id:
            ...
            form.add_error(
                None,
                "FILE_REQUIRED: This document is synced from Google Drive. "
                "Replace the file in the Drive record folder instead.",
            )
            uploaded_file = None
        elif not uploaded_file and not document.file:
            form.add_error(None, "FILE_REQUIRED: Please select a file to upload.")
...
# ---------------------------------------------------------------------------
# Drive record-folder linking (mirrors the Notes tab's Drive folder linking;
# here each PROCEEDING links to one subfolder of the matter's Drive folder,
# and its PDFs sync in as Record documents)
# ---------------------------------------------------------------------------


@login_required
def record_link_modal(request, matter_id):
    """Modal linking this matter's proceedings to Drive record folders."""
    ...
    matter, _ = get_matter_from_url(request, matter_id)
    proceedings = list(
        Proceeding.objects.filter(matter=matter).order_by("-primary", "id")
    )
    folders = records.list_record_subfolders(matter)
    ...
    context = {
        ...
        "matter_linked": bool(matter.drive_folder),
    }
    return render(request, "case/documents/record-link-modal.html", context)


def _queue_resync_proceeding(proceeding):
    """Resync via django-q so linking a large record folder doesn't block."""
    from apps.drive import records

    try:
        from django_q.tasks import async_task

        async_task("apps.drive.records.resync_proceeding_by_id", proceeding.id)
    except Exception:
...
@login_required
@require_POST
def record_link(request, matter_id):
    """Set each proceeding's record folder and sync the changed ones.

    Unlinking never removes synced documents (the record mirror is
    append-only); it just stops future ingestion from that folder.
    """
    from apps.matters.proceedings.models import Proceeding

    matter, _ = get_matter_from_url(request, matter_id)
    proceedings = list(Proceeding.objects.filter(matter=matter))

    chosen = {}
    for proceeding in proceedings:
        chosen[proceeding.id] = request.POST.get(f"folder_{proceeding.id}", "").strip()

    # One folder feeds one proceeding.
    picked = [folder for folder in chosen.values() if folder]
    duplicate = next(
        (folder for folder in set(picked) if picked.count(folder) > 1), None
    )
    if duplicate:
        # 200 so HTMX swaps the message into the modal's error slot.
        return HttpResponse(
            f'<p class="error-text">“{duplicate}” is selected for more than '
            "one proceeding. A record folder can only feed one.</p>"
        )

    changed = []
    for proceeding in proceedings:
        folder = chosen[proceeding.id] or None
        if folder != proceeding.drive_folder:
            proceeding.drive_folder = folder
            proceeding.save()
            if folder:
                changed.append(proceeding)

    for proceeding in changed:
        _queue_resync_proceeding(proceeding)
