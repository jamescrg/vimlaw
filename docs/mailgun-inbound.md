...
email) to `kosmos-intakes@mail.craiglegal.law`. Mailgun POSTs the parsed message to
...
2. **Route.** The Mailgun plan's route quota is 1 and the slot was already
   ...
   - Expression:
     `match_recipient("(billing|kosmos-intakes|kosmos-dev-intakes)@mail\.craiglegal\.law")`
   ...
   - Every matched message goes to BOTH webhooks; each instance only
     ...
     `config/.env`: `kosmos-intakes` on prod, `kosmos-dev-intakes` on dev) and drops the
     rest. Forward to `kosmos-dev-intakes@mail.craiglegal.law` to integration-test
   ...
   - The billing inbox also receives a copy of everything the route matches,
     ...
     `to:(kosmos-intakes@mail.craiglegal.law OR kosmos-dev-intakes@mail.craiglegal.law)` to
   ...
   curl -s --user 'api:<MAILGUN_API_KEY>' https://api.mailgun.net/v3/routes \
     ...
     -F expression='match_recipient("(billing|kosmos-intakes|kosmos-dev-intakes)@mail\.craiglegal\.law")' \
...
Forward any email from a firm address to `kosmos-intakes@mail.craiglegal.law` (or
`kosmos-dev-intakes@mail.craiglegal.law` to test against the dev server). An
